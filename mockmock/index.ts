...
import { isAnyObject, loadFixtureData, writeData } from './utils';

// Utility class, still needed?
class MockEvent {
  constructor(readonly mock: AnyObject) {}
}
...
class Mockmock {
  ...
  private get fixture(): MockFixture {
  ...
  private initContext(contextId: string) {
    ...
    if (isNewContext) {
      this.recording[contextId] = { data: {}, methods: {}, requests: {} };
    }

    // init tracker context if this is the first time we have switched to this context
    if (isNewTracker) {
      this.tracker[contextId] = { data: {}, methods: {}, requests: {} };
    }
  }
   ...
   */
  private checkId(id: string, mockTypeString: string) {
    // init context
    if (!this.recording[this.contextId]) {
      this.recording[this.contextId] = { data: {}, methods: {}, requests: {} };
    }
  ...
  private trackId(id: string, mockTypeString: MockTypeString): number {
    ...
    if (!this.tracker[this.contextId]) {
      this.tracker[this.contextId] = { data: {}, methods: {}, requests: {} };
    }
  ...
  /**
   * Record data directly (without wrapping a method), useful for capturing multiple data chunks
   * caused by a single method call or within event listeners (i.e. WebSocket messages).
   *
   * @param dataId the unique ID representing the data chunk
   * @param data an object
   */
  private recordData(dataId: string, data: unknown): void {
    this.add(dataId, 'data', data);
  }

  /**
   * Record the result of a synchronous method by wrapping it in an outer method that stores the result
   * of the calling the inner (original) method, mapping it to a method id.
   *
   * @param methodId unique ID representing the method
   * @param syncMethodName the name of the synchronous method
   * @param scope the scope (object) that method should be called with
   */
  private recordMethod<T>(
    methodId: string,
    syncMethodName: string,
    scope: object
  ): () => T {
    const originalMethod: () => T = scope[syncMethodName];

    // Wrap the original method
    const fnWrapper = <Type>(...args: Type[]): Type => {
      const result = originalMethod.call(scope, args);
      this.add(methodId, 'methods', result);
      return result;
    };
  ...
  /**
   * Record the result (response) of calling an asynchronous method, mapping it to a request id.
   *
   * @param requestId unique ID representing the request
   * @param asyncMethodName name of the asynchronous method
   * @param scope the scope (object) that method should be called with
   */
  private recordRequest<T>(
    requestId: string,
    asyncMethodName: string,
    scope: object
  ): () => Promise<T> {
    const originalMethod: () => Promise<T> = scope[asyncMethodName];

    // Wrap the original method
    const fnWrapper = async <Type>(...args: Type[]): Promise<Type> => {
      const result = await originalMethod.call(scope, args);
      this.add(requestId, 'requests', result);
  ...
  public frameCount(
    id: string,
    mockTypes: MockTypeString[] = ['requests', 'methods', 'data'],
  ...
  public record<T>(
    ...
    if (typeof methodOrData !== 'function') {
      // handle result (recording data without wrapping)
      ...
      return this.recordRequest<T>(id, methodOrData.name, scope);
    } else if (methodOrData.constructor.name === 'Function' && scope) {
      // handle method (synchronous)
      return this.recordMethod<T>(id, methodOrData.name, scope);
    } else {
